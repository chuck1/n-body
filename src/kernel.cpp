...
void step_collisions(
		...
		CollisionBuffer * cb,//struct Pair* pairs,
		...
		)
{
	...
	/* work group */
	int local_block = cb->_M_size / get_num_groups(0);

	int i_group0 = get_group_id(0) * local_block;
	int i_group1 = i_group0 + local_block;

	if(get_group_id(0) == (get_num_groups(0) - 1)) i_group1 = cb->_M_size;
	...
	{
		struct CollisionBuffer::Pair* pcbp = cb->_M_pairs + p;

		if(!(pcbp->flag & CollisionBuffer::Pair::FLAG_UNRESOLVED)) continue;

		struct Body* b0 = bodies + pcbp->i;
		struct Body* b1 = bodies + pcbp->j;
