...
void			Frame::sphere(float m, float w, float v)
{
	...
	for(unsigned int i = 0; i < bodies_.size(); i++)
	{
		...
		while(1)
		{
			try_insert(&b.x.x, w, w, w, func1, func1, func1, rad, i);
		...
		unsigned int idx)
{
	...
	for(unsigned int j = 0; j < 10; j ++)
	{
		...
		for(unsigned int i = 0; i < idx; i++)
		{
			...
			if(distance(&b.x.x, x) < (b.radius + radius))
			{
...
void			Frame::rings(float m, float w)
{
	...
	for(unsigned int i = 0; i < bodies_.size(); i++)
	{
		...
		b.x.y = ((float)(rand() % 1000000) / 1E6f * 2.0f - 1.0f) * 0.01 * rad;
...
BoundingBox	Frame::get_bounding_box() const
{
	...
	for(Body const & b : bodies_) {
		if(b.alive) {
			bb.a.x = std::min(bb.a.x, b.x.x);
			bb.a.y = std::min(bb.a.y, b.x.y);
			bb.a.z = std::min(bb.a.z, b.x.z);

			bb.b.x = std::max(bb.b.x, b.x.x);
			bb.b.y = std::max(bb.b.y, b.x.y);
			bb.b.z = std::max(bb.b.z, b.x.z);
